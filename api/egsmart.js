...
async function getNanoAlphabet() {
  if (!customAlphabet) {
    const { customAlphabet: imported } = await import("nanoid");
    customAlphabet = imported;
  }
...
module.exports = async (req, res) => {
  ...
  if (action === "login") {
    ...
    const token = jwt.sign(
      { uid: u._id.toString(), user: u.user, role: u.role },
      JWT_SECRET,
      { expiresIn: "12h" }
    );
    return ok(res, { token });
...
# Controller token=${token}`;
  ...
  if (action === "generateCards") {
    ...
    const alphabetFn = await getNanoAlphabet();
    const gen = alphabetFn("ABCDEFGHJKLMNPQRSTUVWXYZ23456789", length);
    const now = new Date();
    const docs = [];
